...
namespace Build
{
    ...
    <PropertyGroup>
        <VersionPrefix>{gitVersion.Version}</VersionPrefix>
            ...
            Target("build", () =>
            {
                RunShell($"dotnet clean {commandBuildArgs} ./StatikProject.sln");
            });
            
            Target("publish", () =>
            {
                RunShell($"dotnet pack {commandBuildArgsWithVersion} --output {ExpandPath("./output")} {ExpandPath("./StatikProject.sln")}");
            });
